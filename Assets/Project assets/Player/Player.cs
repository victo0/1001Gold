...
public class Player : MonoBehaviour {
	...
	void Update () {
		...
		if(findingPlacement) 
		{ //Gère la texture de la tour lors de la constrution
	...
	public void IncrementResourceLimit(ResourceType type, int amount) { //Commande utilisée pour modifier la limite maximum des ressources..
		resourceLimits[type] += amount;
	}

	private bool ResourceTest (ResourceType type, int value, bool sup) {
		if (sup) {
	...
	public void AddUnit(string unitName, Vector3 spawnPoint, Quaternion rotation, Tower nextTower) { //commande utilisée pour faire spawn une unité.
		...
		GameObject testUnit = ResourceManager.GetUnit(unitName);
		Unit testUnitX = (Unit)testUnit.GetComponent <Unit> ();
		if (ResourceTest(ResourceType.Money, testUnitX.cost, true) )
		{
			GameObject newUnit = (GameObject)Instantiate(ResourceManager.GetUnit(unitName), spawnPoint, rotation);
			Unit newUnitX = (Unit)newUnit.GetComponent <Unit> ();
			int negativeMoney = 0 - newUnitX.cost;
			AddResource (ResourceType.Money, negativeMoney);
			if (newUnitX.control)
			{
				Debug.Log("test");
	...
	public void CreateBuilding(string buildingName, Vector3 buildPoint, Building creator, Rect playingArea) { //commande utilisée pour créer un batiment.
		...
		if (ResourceTest(ResourceType.Money, tempBuilding.cost, true) )
		{
			if (tempBuilding) {
	...
	public void StartConstruction() { //commande appellée pour lancer une construction
		...
		int negativeMoney = 0 - tempBuilding.cost;
		AddResource (ResourceType.Money, negativeMoney);
	}
